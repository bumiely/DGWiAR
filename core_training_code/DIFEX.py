...
from alg.modelopera import get_fea,get_fea_defix
...
#             loss.backward()
#             # condition1
#             if ((epoch + 1) % (int(self.args.steps_per_epoch * self.args.max_epoch * 0.7)) == 0 or
#                 (epoch + 1) % (int(self.args.steps_per_epoch * self.args.max_epoch * 0.9)) == 0) and \
#                     (not self.args.schuse):
#                 for param_group in opt1.param_groups:
#                     param_group['lr'] = param_group['lr'] * 0.1
#             # condition 2
#             # if (epoch + 1) % self.args.steps_per_epoch * 3 == 0 and not self.args.schuse:
#             #     for param_group in opt1.param_groups:
...
#         super(DIFEX, self).__init__(args)
#         self.args = args
#         self.featurizer = get_fea(args)  # Feature extractor for image data
#         self.bottleneck = common_network.feat_bottleneck(
#             self.featurizer.in_features, args.bottleneck, args.layer)
#         self.classifier = common_network.feat_classifier(
#             args.num_classes, args.bottleneck, args.classifier)
...
#             args.num_classes, self.tfbd, args.classifier)
#         self.teaNet = nn.Sequential(
#             self.teaf,   # out-shape N,512
#             self.teab,   # out-shape N,128
#             self.teac    # out-shape N,class_num
#         )
#
#     def teanettrain(self, dataloaders, n_steps, opt1, sch1):
#         self.teaNet.train()
#         # 多个s-domain的infinite-dataloader以列表形式存在dataloaders中
#         minibatches_iterator = zip(*dataloaders)
#         for step in range(n_steps):
#             minibatches = [(tdata) for tdata in next(minibatches_iterator)]
#             # 所有s-domain的数据混合成一个大的
#             all_x = torch.cat([data[0].cuda().float() for data in minibatches])
#             all_y = torch.cat([data[1].cuda().long() for data in minibatches])
#
#             # Directly process images
#             all_p = self.teaNet(all_x)
#             loss = F.cross_entropy(all_p, all_y, reduction='mean')
#
#             opt1.zero_grad()
#             loss.backward()
#             # condition1
#             if ((step + 1) % (int(self.args.steps_per_epoch * self.args.max_epoch * 0.7)) == 0 or
#                 (step + 1) % (int(self.args.steps_per_epoch * self.args.max_epoch * 0.9)) == 0) and \
#                     (not self.args.schuse):
#                 for param_group in opt1.param_groups:
#                     param_group['lr'] = param_group['lr'] * 0.1
#             # condition 2
#             # if (step + 1) % self.args.steps_per_epoch*3 == 0 and not self.args.schuse:
#             #     for param_group in opt1.param_groups:
#             #         param_group['lr'] = param_group['lr'] * 0.05
#
#             opt1.step()
...
#                 sch1.step()
#
#             if step % int(self.args.steps_per_epoch) == 0:
#                 print('step: %d, cls loss: %.4f' % (step, loss))
#         self.teaNet.eval()
#
#     def coral(self, x, y):
#         mean_x = x.mean(0, keepdim=True)
...
#
#         # Distillation loss
#         # 计算的是 all_z[:, :self.tfbd] 和 tfea 对应位置元素的均方误差
#         loss2 = F.mse_loss(all_z[:, :self.tfbd], tfea) * self.args.alpha
#         if self.args.disttype == '2-norm':
#             loss3 = -F.mse_loss(all_z[:, :self.tfbd],
#                                 all_z[:, self.tfbd:]) * self.args.beta
...
#         if sch:
#             sch.step()
#         return {'class': loss1.item(), 'dist': (loss2).item(), 'exp': (loss3).item(), 'align': loss4.item(),
#                 'total': loss.item()}
#
#     def predict(self, x):
#         return self.classifier(self.bottleneck(self.featurizer(x)))
...
class DIFEX(Algorithm):
    def __init__(self, args):
        super(DIFEX, self).__init__(args)
        self.args = args
        self.featurizer = get_fea_defix(args)  # Feature extractor for image data
        self.bottleneck = common_network.feat_bottleneck(
            self.featurizer.in_features, args.bottleneck, args.layer)
        self.classifier = common_network.feat_classifier(
            args.num_classes, args.bottleneck, args.classifier)
    ...
    def teanettrain(self, dataloaders, n_steps, opt1, sch1,eval_loaders,eval_name_dict,alg):
        acc_record = {}
        ...
        for step in range(n_steps):
            ...
            # condition 2
            # condition 3
            if ((step + 1) % (int(self.args.steps_per_epoch * self.args.max_epoch * 0.3)) == 0 or
                (step + 1) % (int(self.args.steps_per_epoch * self.args.max_epoch * 0.6)) == 0 or
                (step + 1) % (int(self.args.steps_per_epoch * self.args.max_epoch * 0.9)) == 0) and \
                (not self.args.schuse):
                for param_group in opt1.param_groups:
                    param_group['lr'] = param_group['lr'] * 0.1
            ...
            if step % int(self.args.steps_per_epoch) == 0 and step !=0:
                print('step: %d, cls loss: %.4f' % (step, loss))
                # 验证一下学习效果
                # s = ''
                # for item in acc_type_list:
                #     acc_record[item] = np.mean(np.array([modelopera.accuracy_tea(
                #         alg, eval_loaders[i]) for i in eval_name_dict[item]]))
    ...
    def predict_tea(self, x):
        return self.teac(self.teab(self.teaf(x)))
